package com.aianik.anik.ai.openapi.client.core;
...
/**
 * Anik AI OpenAPI Fluent Builder facade
 * <p>
 * Usage example:
 * <pre>
 * // streaming conversation
 * AnikAiOpenApi.chat(agentId)
 ...
 * ChatSyncResponse resp = AnikAiOpenApi.chat(agentId)
 *     .conversationId("conv-123")
 *     .content("Hello")
 *     .execute();
 * </pre>
...
public class AnikAiOpenApi {
    ...
    public static class ChatBuilder {
        ...
        public ChatBuilder listener(SseEventListener listener) {
        ...
        public void stream() {
            checkClient();
            chatClient.chatStream(buildRequest(), listener);
        }
        ...
        public OpenApiChatSyncResponse execute() {
            ...
            if (result.getStatus() != 1) {
                throw new AnikAiException("Chat failed: " + result.getMessage());
            }
        ...
        private void checkClient() {
            if (chatClient == null) {
                throw new IllegalStateException("AnikAiOpenApi not initialized. Use @EnableAnikAiOpenApi or call AnikAiOpenApi.init()");
            }
        }
    }
    ...
    public static Result<List<OpenApiAgentVO>> listAgents() {
        if (agentClient == null) {
            throw new IllegalStateException("AnikAiOpenApi not initialized");
        }
    ...
    public static class UserRegisterBuilder {
        ...
        public OpenApiUserVO execute() {
            ...
            if (result.getStatus() != 1) {
                throw new AnikAiException("User register failed: " + result.getMessage());
            }
            return result.getData();
        }
    }
