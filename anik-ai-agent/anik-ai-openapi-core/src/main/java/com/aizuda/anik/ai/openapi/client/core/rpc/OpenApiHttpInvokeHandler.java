...
import com.aizuda.anik.ai.openapi.client.core.annotation.PathParam;
import com.aizuda.anik.ai.openapi.client.core.annotation.QueryParam;
import com.aizuda.anik.ai.openapi.client.core.listener.SseEventListener;
import com.aizuda.anik.ai.openapi.client.core.config.AnikAiOpenApiProperties;
...
/**
 * OpenAPI HTTP dynamic proxy handler
 * <p>
 * Based on JDK dynamic proxy, parse @OpenApiMapping annotation,
 * Automatically build and send HTTP requests, supporting SSE streaming responses.
 *
 * @author openanik
 * @date 2026-04-24
 */
public class OpenApiHttpInvokeHandler implements InvocationHandler {
    ...
    private final AnikAiOpenApiProperties openApiProperties;
    private final AnikAiAgentProperties agentProperties;
    private final String baseUrl;

    public OpenApiHttpInvokeHandler(HttpClient httpClient, AnikAiOpenApiProperties openApiProperties, AnikAiAgentProperties agentProperties) {
    ...
    /**
     * Build base URL
     * The configuration of openApiProperties will be used first. If not configured, agentProperties will be used.
     */
    private String buildBaseUrl() {
        ...
        //Remove the trailing slash from host
        host = host.replaceAll("/$", "");

        // Build the complete URL: protocol://host:port/prefix
        StringBuilder url = new StringBuilder(protocol).append("://").append(host);

        // Only add port number if it is not the default port
        if ((openApiProperties.isHttps() && port != 443) || (!openApiProperties.isHttps() && port != 80)) {
            url.append(":").append(port);
        }

        // Add prefix (make sure it starts with /)
    ...
    /**
     * Build the complete request URL (including path and query parameters)
     */
    ...
    /**
     * Build a basic HTTP request (including common request headers and timeout configuration)
     */
    private HttpRequest.Builder baseRequest(String url) {
        return HttpRequest.newBuilder()
                ...
                .header("Anik-Ai-App-Id", agentProperties.getAppId())
                .header("Anik-Ai-Token", agentProperties.getToken());
    }

    /**
     * Perform normal HTTP requests (non-streaming)
     */
    private Object executeRequest(String url, OpenApiMapping.HttpMethod method,
        ...
        if (response.statusCode() >= 400) {
            ...
            throw new AnikAiException("OpenAPI request failed: HTTP " + response.statusCode()
                    + " - " + response.body());
        }
    ...
    /**
     * Perform SSE streaming requests
     */
    ...
    /**
     * Serialize request body object to HTTP BodyPublisher
     */
    private HttpRequest.BodyPublisher bodyPublisher(Object body) {
        ...
        try {
            ...
            throw new AnikAiException("Failed to serialize request body", e);
        }
    }
}
