...
import com.aianik.anik.ai.features.resource.config.ResourceConfig;
import com.aianik.anik.ai.features.resource.enums.ResourceStorageTypeEnum;
import com.aianik.anik.ai.features.resource.util.MimeTypeUtils;
import com.aianik.anik.ai.features.resource.util.StorageKeyGenerator;
import io.minio.*;
...
public class MinioResourceStorageService implements ResourceStorageService {
    ...
    public String store(String bizType, String fileName, InputStream inputStream) {
        try {
            ...
            throw new AnikAiException("Failed to store resource to MinIO: " + fileName, e);
        }
    }
    ...
    public InputStream load(String storageKey) {
        try {
            ...
            throw new AnikAiException("Failed to load resource from MinIO: " + storageKey, e);
        }
    }
    ...
    public boolean exists(String storageKey) {
        try {
            ...
            throw new AnikAiException("Failed to check resource existence in MinIO", e);
        }
    }
            ...
            synchronized (this) {
                if (minioClient == null) {
                    ...
                    if (cfg.getEndpoint() == null || cfg.getEndpoint().isBlank()) {
                        throw new IllegalStateException("MinIO is not configured. Set anik-ai.resource.minio.*");
                    }
    ...
    private void ensureBucketExists() {
        try {
            ...
            throw new AnikAiException("Failed to ensure MinIO bucket exists", e);
        }
    }
}
