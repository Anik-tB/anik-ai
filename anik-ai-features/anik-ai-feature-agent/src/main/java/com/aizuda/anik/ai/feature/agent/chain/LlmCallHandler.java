...
import com.aizuda.anik.ai.feature.agent.persist.ChatResultPersistService;
...
/**
 * End point of the chain of responsibility: Assemble {@link com.aizuda.anik.ai.common.dto.agent.ChatDispatchRequest} from {@link AgentChatContext} and call the remote Client via gRPC streaming.
 */
@Slf4j
@Component
@Order(80)
...
public class LlmCallHandler implements AgentChatHandler {
    ...
    public void handle(AgentChatContext ctx) {
        ...
        if (ctx.getTargetClient() == null) {
            sendError(ctx.getEmitter(), "No available client instance found");
            ctx.setTerminated(true);
            return;
        }

        if (ctx.getModelConfig() == null) {
            sendError(ctx.getEmitter(), "Model configuration missing");
            ctx.setTerminated(true);
            return;
        }

        com.aizuda.anik.ai.common.dto.agent.ChatDispatchRequest request = buildDispatchRequest(ctx);
