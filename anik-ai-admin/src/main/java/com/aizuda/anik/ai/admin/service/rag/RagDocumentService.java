...
import com.aizuda.anik.ai.features.rag.dedup.DedupResult;
import com.aizuda.anik.ai.features.rag.dedup.DocumentDedupChecker;
import com.aizuda.anik.ai.features.rag.dedup.UploadDecision;
import com.aizuda.anik.ai.features.rag.enums.DedupAction;
import com.aizuda.anik.ai.features.rag.enums.DedupStrategy;
import com.aizuda.anik.ai.features.rag.enums.DocumentSourceTypeEnum;
import com.aizuda.anik.ai.features.resource.ResourceService;
import com.aizuda.anik.ai.features.resource.enums.ResourceBizTypeEnum;
...
import com.aizuda.anik.ai.features.rag.enums.RagDocumentStatus;
...
import com.aizuda.anik.ai.admin.vo.rag.RagChunkCreateRequestVO;
import com.aizuda.anik.ai.admin.vo.rag.RagChunkQueryVO;
import com.aizuda.anik.ai.admin.vo.rag.RagChunkResponseVO;
import com.aizuda.anik.ai.admin.vo.rag.RagChunkUpdateRequestVO;
...
import com.aizuda.anik.ai.features.rag.pipeline.DocumentPipeline;
import com.aizuda.anik.ai.features.rag.strategy.importer.DocumentImportFactory;
import com.aizuda.anik.ai.features.rag.strategy.importer.DocumentImportStrategy;
import com.aizuda.anik.ai.features.rag.strategy.importer.ImportRequest;
import com.aizuda.anik.ai.features.rag.strategy.importer.ImportResult;
...
public class RagDocumentService {
    ...
    /**
     * Clean documents and their associated resources (vectors/repositories/chunks/document lines).
     * It is also used for common delete and OVERWRITE upload link reuse.
     */
    private void cleanupDocumentInternal(Long documentId) {
        ...
        // Clean up resource library files
    ...
    public void reprocess(Long documentId) {
        RagDocumentPO doc = ragDocumentMapper.selectById(documentId);
        if (doc == null) {
            throw new AnikAiException("Document not found: " + documentId);
        }
    ...
    public RagChunkResponseVO getChunkDetail(Long chunkId) {
        ...
        if (chunk == null) {
            throw new AnikAiException("Chunk not found: " + chunkId);
        }
    ...
    public RagChunkResponseVO createChunk(RagChunkCreateRequestVO request) {
        ...
        if (request.getDocumentId() == null) {
            throw new AnikAiException("documentId is required");
        }
        if (StrUtil.isBlank(request.getContent())) {
            throw new AnikAiException("content is required");
        }
        ...
        try {
            ...
            log.warn("New slice vectorization failed, chunkId={}", chunk.getId(), e);
        }
    ...
    public RagChunkResponseVO updateChunk(Long chunkId, RagChunkUpdateRequestVO request) {
        ...
        if (knowledge != null) {
            try {
                ...
                log.warn("Edit slice vectorization failed, chunkId={}", chunkId, e);
            }
        }
    ...
    /**
     * Upload link core: completes the two stages of "judgment → writing" (no distributed lock).
     */
    ...
    private RagDocumentResponseVO doSaveAndProcess(RagPO knowledge, ImportResult result,
        ...
        return switch (decision.getType()) {
            ...
            case REJECT -> throw new AnikAiCommonException(buildConflictMessage(decision));
    ...
    /**
     * Overlay upload: clean up old documents (including chunks/vectors/resources) and re-enter the database as new files.
     * conflictDocumentId still points to the replaced old ID to facilitate front-end feedback.
     */
    ...
    /**
     * Directly drop a line into RagDocument and return the response VO.
     * For reuse by regular upload and preview-commit links, the caller must prepare the resource ID by itself.
     */
    ...
    /**
     * Clean documentation (public version). The OVERWRITE of the preview-commit process is called directly without @Transactional packaging.
     */
    ...
    /**
     * Deduplication evaluator exposed to PreviewService.
     */
    ...
    private String buildConflictMessage(UploadDecision decision) {
        ...
        return switch (decision.getMatchType()) {
            case BY_NAME -> "A document with the same name already exists in this knowledge base" + suffix;
            case BY_CONTENT -> "A document with the same content already exists in this knowledge base" + suffix;
            case BOTH -> "A document with the same name and content already exists in this knowledge base" + suffix;
            case NONE -> "Upload rejected" + suffix;
        };
    }
    ...
    private RagPO requireKnowledge(Long ragId) {
        ...
        if (knowledge == null) {
            throw new AnikAiException("Knowledge not found: " + ragId);
        }
        return knowledge;
    }
