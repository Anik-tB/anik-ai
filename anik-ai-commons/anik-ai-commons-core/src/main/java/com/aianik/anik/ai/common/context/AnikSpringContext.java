package com.aianik.anik.ai.common.context;
 ...
 */
@Component("anikAiSpringContext")
@Order(Ordered.HIGHEST_PRECEDENCE)
@Slf4j
public class AnikSpringContext implements BeanFactoryPostProcessor, ApplicationContextAware {
    ...
    public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) {
        AnikSpringContext.FACTORY = beanFactory;
    }

    @Override
    public void setApplicationContext(ApplicationContext applicationContext) {
        AnikSpringContext.CONTEXT = applicationContext;
    }

    public static ListableBeanFactory getBeanFactory() {
        final ListableBeanFactory factory = null == FACTORY ? CONTEXT : FACTORY;
        if (null == factory) {
            throw new AnikAiException("No ConfigurableListableBeanFactory or ApplicationContext injected, maybe not in the Spring environment?");
        }
        return factory;
    }
